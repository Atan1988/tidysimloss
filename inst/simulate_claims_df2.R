...
claims_df <- policy_df_w_claims %>% dplyr::pull(total_claims ) %>%
  purrr::map(~seq(1, ., 1)) %>%
  ...
  dplyr::mutate(
    lim_exceed = ifelse(loss > limit, 1, 0),
...
claims_df1 <- claims_df %>%
  dplyr::select(
    dplyr::one_of(colnames(frq_data_net)), loss, limit, lim_exceed
  ) %>%
...
full_data <- dplyr::bind_rows(
  ...
  dplyr::mutate(
    loss = ifelse(freq2 == 1, ded + 0.1, loss),
  ) %>%
  dplyr::mutate_at(
...
brmsfit <- brm( formula = mvfit_formula,
                data = full_data,
                prior = c(
                  prior(normal(-1, 0.1), class = 'b', resp = 'claimcount', coef = 'Intercept', nlpar = 'f1'),
                  prior(normal(5, 2), class = 'b', resp = 'loss', nlpar = 's1')
                ), empty = TRUE)
brmsfit$fit <- stanfit
